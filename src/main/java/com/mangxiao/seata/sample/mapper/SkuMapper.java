...
public interface SkuMapper {
    ...
    @Select("UPDATE sku SET sku_name=#{skuName} WHERE id = #{sku.id}")
    int update(Sku sku);

    @Select("DELETE FROM sku WHERE id = #{id}")
    int delete(Long id);

    @Select("INSERT INTO sku (sku_name,create_time,update_time,create_user_id,update_user_id,yn) VALUES(#{skuName},#{createTime},#{updateTime},#{createUserId},#{updateUserId},#{yn})")
    int add(Sku sku);
}
